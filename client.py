...
class LoginWindow(CTk):
    def __init__(self):
        ...
        CTkLabel(self, text = "Ваше ім'я").pack(pady = 5)
        self.name_entry = CTkEntry(self)
        self.name_entry.pack(pady = 5)

        CTkLabel(self,text = "Хост:").pack(pady = 5)
        self.host_entry = CTkEntry(self)
        self.host_entry.insert(0, "0.tcp.eu.ngrok.io")
        ...
        CTkButton(self,text= "Підключитись", command=self.start_chat).pack(pady=15)

    def start_chat(self):
            name = self.name_entry.get().strip() or "Used"
            host = self.host_entry.get().strip()
            port = int(self.port_entry.get().strip())
            self.destroy()
            win = MainWindow(name, host, port)
            win.mainloop()
...
class MainWindow(CTk):
    def __init__(self, name, host, port):
        ...
        self.btn = CTkButton(self, text='Налаштування', command=self.toggle_show_menu, width=30)
        self.btn.place(x=0, y=0)
        self.menu_show_speed = 10
        ...
        self.send_button = CTkButton(self.bottom_frame, text='Відправіть повідомлення', width=40, command=self.send_message)
